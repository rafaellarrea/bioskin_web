...
export class AppointmentStateMachine {
  ...
  async _handleAwaitingDate(userMessage) {
    ...
    if (!date) {
      // Check interruption
      const interruption = await this._checkInterruption(userMessage, 'date');
      if (interruption) {
        return {
          state: this.state,
          message: `${interruption.message}\n\nEntonces, ¿qué día te gustaría agendar?`
        };
      }
  ...
  async _handleAwaitingTime(userMessage) {
    ...
    if (!time) {
      // Check interruption
      const interruption = await this._checkInterruption(userMessage, 'time');
      if (interruption) {
        return {
          state: this.state,
          message: `${interruption.message}\n\nEntonces, ¿a qué hora te gustaría?`
        };
      }
  ...
  async _handleAwaitingName(userMessage) {
    ...
    if (!isValidName(userMessage)) {
      // Check interruption
      const interruption = await this._checkInterruption(userMessage, 'name');
      if (interruption) {
        return {
          state: this.state,
