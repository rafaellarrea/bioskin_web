...
export function getPool() {
  ...
  const rawConnectionString = process.env.CLINICAL_DATABASE_URL || process.env.NEON_DATABASE_URL || process.env.POSTGRES_URL;
  const connectionString = rawConnectionString ? rawConnectionString.trim() : null;

  if (!connectionString) {
    console.warn('⚠️ CLINICAL_DATABASE_URL, NEON_DATABASE_URL ni POSTGRES_URL están configuradas. La funcionalidad de Fichas Clínicas no funcionará correctamente.');
    return null;
  }
