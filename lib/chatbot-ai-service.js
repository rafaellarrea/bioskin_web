...
⚠️ IMPORTANTE: Mantenga siempre el tratamiento de "usted". Responda profesionalmente adaptándose a cada conversación.`;
  ...
  async generateResponse(userMessage, conversationHistory = [], calendarTools = null) {
    try {
      ...
      // ⏰ INYECTAR HORA ACTUAL DE ECUADOR (CRÍTICO para saludos correctos)
      ...
      const timeContext = `
⏰ HORA ACTUAL EN ECUADOR: ${hour.toString().padStart(2, '0')}:${minutes.toString().padStart(2, '0')}
📅 Fecha: ${ecuadorDate.toLocaleDateString('es-ES', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}

⚠️ SALUDO CORRECTO PARA USAR AHORA: "${currentGreeting}"

IMPORTANTE: Use EXACTAMENTE "${currentGreeting}" en su respuesta de saludo, NO "Buenos días" ni otro.
`;
      ...
      console.log(`⏰ [AI] Hora Ecuador inyectada: ${hour}:${minutes} → Saludo: ${currentGreeting}`);
