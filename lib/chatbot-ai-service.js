...
function getOpenAIClient() {
  if (!openai) {
    ...
    openai = new OpenAI({
      ...
      timeout: 4000, // Timeout de 4s para requests
...
Para agendar citas o info detallada, pide que contacten directamente.`;
  ...
  async generateResponse(userMessage, conversationHistory = []) {
    try {
      ...
      // Llamar a OpenAI con timeout muy agresivo (4s total)
      ...
      console.log('🔍 [AI] Creando promesa de timeout (4.5s)...');
      const timeoutPromise = new Promise((_, reject) => {
        const timer = setTimeout(() => {
          console.log('⏰ [AI] ¡TIMEOUT alcanzado! (4.5s)');
      ...
      console.log('⏳ Esperando respuesta de OpenAI (timeout: 4.5s)...');
