...
export async function handleFinanceMessage(phone, message, type, entryData) {
  ...
  // 2. Manejo de IMAGEN (Inicio de flujo)
  if (type === 'image') {
    // Obtener imagen
    const imageId = entryData?.entry?.[0]?.changes?.[0]?.value?.messages?.[0]?.image?.id;
    if (!imageId) return null;

    // Notificar análisis en proceso
    // (Esto requiere enviar mensaje intermedio, pero en webhook return no se puede. 
    // asumiremos que WhatsApp bot enviará la respuesta final)
    
    // Obtener URL de descarga (requiere token de WhatsApp)
    const token = process.env.WHATSAPP_ACCESS_TOKEN;
    if (!token) {
      console.error('❌ Missing WHATSAPP_ACCESS_TOKEN');
      return { handled: true, response: "Error de configuración: falta token de WhatsApp." };
    }

    try {
      // 1. Obtener URL
      const mediaUrlResp = await axios.get(`https://graph.facebook.com/v18.0/${imageId}`, {
      ...
      // 3. Análisis IA
      const analysis = await analyzeInvoiceImage(imageBuffer);
      
      if (!analysis.is_invoice) {
        return { handled: true, response: "❌ La imagen no parece ser una factura o no es legible. Intenta tomar la foto más clara." };
      }
