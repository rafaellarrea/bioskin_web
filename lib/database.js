...
export const blogQueries = {
  // Función para obtener todos los datos para backup
  getBackupData: () => {
    return {
      blogs: db.prepare('SELECT * FROM blogs').all(),
      tags: db.prepare('SELECT * FROM tags').all(),
      blog_tags: db.prepare('SELECT * FROM blog_tags').all(),
      citations: db.prepare('SELECT * FROM citations').all()
    };
  },
  
  // Insertar nuevo blog con todos los campos
  insertBlog: db.prepare(`
    INSERT INTO blogs (
      title, slug, excerpt, content, category, blog_type, author, 
      published_at, read_time, image, featured, week_year, 
      is_ai_generated, ai_prompt_version
    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `),
...
export function createCompleteBlog(blogData, tags, citations) {
  const transaction = db.transaction(() => {
    ...
    // 1. Insertar blog con todos los campos requeridos
    const blogResult = blogQueries.insertBlog.run(
      ...
      blogData.content,
      blogData.category,
      blogData.category, // blog_type = category
      blogData.author,
      ...
      getCurrentWeekYear(),
      1, // is_ai_generated
      'v2.0-production' // ai_prompt_version
    );
