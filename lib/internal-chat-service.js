...
export async function processInternalChatMessage({ message, sessionId, isNewSession, isNewPatient, mode }) {
  ...
  try {
    ...
    if (mode === 'assistant') {
      ...
      // A. Calendar Context
      if (lowerMsg.includes('agenda') || lowerMsg.includes('cita') || lowerMsg.includes('horario') || lowerMsg.includes('disponible') || lowerMsg.includes('calendario') || lowerMsg.includes('mañana') || lowerMsg.includes('hoy')) {
        try {
