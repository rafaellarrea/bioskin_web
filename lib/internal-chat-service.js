...
export async function processInternalChatMessage({ message, sessionId, isNewSession, isNewPatient, mode }) {
  ...
  try {
    ...
    // 5. Call Gemini (Using 1.5 Flash for better stability/limits)
    ...
    if (!response.ok) {
      ...
      if (response.status === 429) {
        throw new Error('El sistema de IA está saturado momentáneamente (Límite de cuota). Por favor intenta en unos minutos.');
      }
