...
export async function processInternalChatMessage({ message, sessionId, isNewSession, isNewPatient, mode }) {
  ...
  try {
    ...
    // Primary: gemini-1.5-flash (Standard, fast, cost-effective)
    // Backups: gemini-1.5-flash-8b (Cheaper/Faster), gemini-1.0-pro (Legacy stable), gemini-1.5-pro (Higher intelligence)
    const models = ['gemini-1.5-flash', 'gemini-1.5-flash-8b', 'gemini-1.0-pro', 'gemini-1.5-pro'];
    let aiText = null;
    let lastError = null;
