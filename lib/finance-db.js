...
export async function saveFinanceRecord(record) {
  ...
  let clinicValue = record.clinic || 'BIOSKIN';
  if ((record.total_payment === 0 && record.expenses > 0) && (!record.clinic || record.clinic === 'BIOSKIN')) {
     ...
     // The AI prompt sets clinic default to 'BIOSKIN'. We should check if the AI *found* a clinic or if we are falling back.
