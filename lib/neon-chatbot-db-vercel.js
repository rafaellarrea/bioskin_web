// Configuración usando @neondatabase/serverless con configuración optimizada para Vercel
...
console.log('✅ Cliente Neon inicializado con fetchConnectionCache');
...
export async function upsertConversation(sessionId, phoneNumber) {
  ...
  try {
    console.log('🔄 Ejecutando query SQL (Neon)...');
    ...
    if (result && result.length > 0) {
      console.log(`✅ Conversación actualizada, ID: ${result[0].id}`);
      return result[0];
    }
...
export async function saveMessage(sessionId, role, content, tokensUsed = 0, messageId = null) {
  ...
  try {
    ...
    if (result && result.length > 0) {
      console.log(`✅ Mensaje guardado, ID: ${result[0].id}`);
      return result[0];
    }
