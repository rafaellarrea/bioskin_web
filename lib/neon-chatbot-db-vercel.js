...
export async function upsertConversation(sessionId, phoneNumber) {
  ...
  // Warm-up query: despertar la base de datos con una query simple
  try {
    console.log('🔥 Warm-up: despertando base de datos...');
    await sql`SELECT 1 as warmup`;
    console.log('✅ Base de datos despierta');
  } catch (warmupError) {
    console.warn('⚠️ Warm-up falló (continuando):', warmupError.message);
  }
