...
Tono: Profesional, servicial, sin presionar.`;
    ...
    // PASO 2: PRIORIDAD ALTA - Detectar PROBLEMA TÉCNICO específico (equipo defectuoso/dañado)
    const hasTechnicalProblem = /(defectuoso|dañado|roto|no funciona|no enciende|error|falla|problema|averiado|malogrado).*equipo/i.test(lastUserMsg) ||
                                /equipo.*(defectuoso|dañado|roto|no funciona|no enciende|error|falla|problema|averiado|malogrado)/i.test(lastUserMsg) ||
                                /(tengo|mi).*(equipo|dispositivo|aparato).*(defectuoso|dañado|problema|falla)/i.test(lastUserMsg);
    
    if (hasTechnicalProblem) {
      console.log('🚨 [TechnicalAI] PROBLEMA TÉCNICO DETECTADO - Respuesta prioritaria');
      
      const greeting = getTimeBasedGreeting();
      
      return {
        responseText: `${greeting}, soy Salomé de BIOSKIN 😊\n\nEntiendo, tiene un equipo con problemas técnicos. Nuestro servicio de reparación y diagnóstico puede ayudarle.\n\n🔧 *Servicios de reparación:*\n• Diagnóstico completo del problema\n• Reparación especializada\n• Reemplazo de componentes\n• Garantía del servicio\n\n*Nota:* Atendemos equipos de cualquier marca o proveedor.\n\n¿Le gustaría que el Ing. Rafael le contacte para evaluar su equipo y brindarle un diagnóstico?`,
    ...
    // PASO 3: Verificar si es pregunta sobre SERVICIO TÉCNICO general (soporte, instalación, garantía, revisión)
    // Esto es DIFERENTE a preguntar por equipos específicos
    const isServiceInquiry = /(ofrecen|tienen|dan|brindan|hay|hacen).*servicio.*t[eé]cnico/i.test(lastUserMsg) ||
                            /servicio.*t[eé]cnico/i.test(lastUserMsg) ||
                            /(revis|manteni|repar|install|soport).*equipo/i.test(lastUserMsg) ||
                            /(pueden|puede).*revisar.*equipo/i.test(lastUserMsg);
    
    if (isServiceInquiry) {
      console.log('🔧 [TechnicalAI] Consulta sobre SERVICIO TÉCNICO detectada');
      
      // Generar saludo contextual
      const greeting = getTimeBasedGreeting();
      
      return {
        responseText: `${greeting}, soy Salomé de BIOSKIN 😊\n\nSí, ofrecemos servicio técnico completo para equipos médicos estéticos. Esto incluye:\n\n🔧 Instalación y configuración\n🔍 Revisión y diagnóstico\n⚙️ Mantenimiento preventivo\n🛠️ Reparación y soporte\n\n*Importante:* Realizamos revisiones de equipos incluso si no fueron adquiridos con nosotros.\n\n¿Le gustaría que el Ing. Rafael le contacte para coordinar una revisión?`,
        suggestedActions: ['offer_engineer_contact'],
        meta: {
