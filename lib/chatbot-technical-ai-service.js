...
Tono: Profesional, servicial, sin presionar.`;
    ...
    if (/(disponible|stock|tienen|cuáles|qué equipos|mostrar|ver)/i.test(lastUserMsg) || technicalMessagesCount === 2) {
      console.log('📋 [TechnicalAI] Mostrando lista de equipos en stock');
      
      const stockList = getStockListForChat();
      const responseText = `${stockList}\n¿Sobre cuál de estos equipos le gustaría más información? 💡`;
    ...
    if (specificEquipment) {
      ...
      const productDetails = formatProductForChat(specificEquipment, true);
      const stockInfo = getStockInfo(specificEquipment.name);
      
      let responseText = productDetails;
      
      // Agregar pregunta contextual según el subtipo
      if (classification.subtype === 'sales') {
        responseText += `\n¿Desea que le envíe una cotización formal o prefiere más detalles técnicos? 📊`;
