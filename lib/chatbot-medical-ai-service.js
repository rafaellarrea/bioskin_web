...
User: "¿La consulta está incluida en el precio?"
→ kind: medical, subtype: policy_inquiry, topic: consultation_fee, confidence: 0.95, needsConsultation: false

User: "¿Tengo que pagar antes?"
→ kind: medical, subtype: policy_inquiry, topic: payment, confidence: 0.95, needsConsultation: false
...
→ kind: technical, subtype: sales, question: promotions, confidence: 0.95, needsConsultation: false
...
6. Si es sobre EQUIPOS médicos (compra, falla, reparación, PROMOCIONES DE EQUIPOS):
...
10. Si pregunta sobre POLÍTICAS, PAGOS, o si la consulta está INCLUIDA:
   → kind: medical, subtype: policy_inquiry

11. Clasifica el subtipo:
   ...
   - appointment_request: quiere agendar cita (INTENCIÓN CLARA DE AGENDAR)
   - policy_inquiry: preguntas sobre políticas, pagos, inclusión de consulta
...
12. Identifica el problema o tratamiento mencionado
...
RESPONDE EN FORMATO JSON PURO (sin markdown):
{
  ...
  "subtype": "skin_concern|treatment_interest|treatment_recommendation|treatment_inquiry|price_inquiry|promotion_inquiry|appointment_request|policy_inquiry|location_inquiry|schedule_inquiry|contact_inquiry|other",
...
function classifyMedicalFallback(message) {
  ...
  if (/(tengo|sufro|me salen|me aparecen).*mancha/i.test(lowerMsg)) {
    ...
    needsConsultation = true;
  } else if (/(incluye|cobran|pago|pagar|forma de pago|tarjeta|efectivo|aceptan)/i.test(lowerMsg)) {
...
export async function generateMedicalReply(classification, conversationHistory = [], tools = null) {
  ...
  try {
    ...
    // Detectar menciones de precio en el mensaje del usuario para búsqueda inteligente
    const priceMatch = lastUserMsg.match(/(\d+)\s*(dolares|dólares|usd|\$)/i) || lastUserMsg.match(/(\$)\s*(\d+)/);
    const mentionedPrice = priceMatch ? parseInt(priceMatch[1] || priceMatch[2]) : null;
    let priceContextFound = null;
    ...
    availableServices.forEach((service, index) => {
      ...
      if (service.promotion && service.promotion.active) {
        ...
        if (now >= validFrom && now <= validUntil) {
          ...
          // Lógica de coincidencia de precio inteligente
          if (mentionedPrice) {
            // Buscar si el precio mencionado coincide con algún precio de la promo (single o double)
            const promoText = JSON.stringify(promo).toLowerCase();
            if (promoText.includes(mentionedPrice.toString())) {
              priceContextFound = `💡 POSIBLE COINCIDENCIA: El usuario mencionó "$${mentionedPrice}". Esto coincide con la promoción de "${service.title}" (${promo.displayMessage}). Es probable que se refiera a este servicio.`;
            }
          }
        }
      }
    ...
    if (activePromotions.length > 0) {
      let promotionsHeader = '🎉 PROMOCIONES ACTIVAS (PRIORIDAD ALTA):\n\n';
    ...
    // Inyectar contexto de precio encontrado si existe
    if (priceContextFound) {
      servicesContext = `⚠️ ATENCIÓN - PISTA DE CONTEXTO:\n${priceContextFound}\n\n` + servicesContext;
    }
...
🔍 **REGLA DE ORO - BÚSQUEDA INTELIGENTE**:
   - Si el usuario menciona una categoría general (ej: "Bioestimuladores"), NO te limites al servicio que se llama exactamente así.
   - BUSCA en todo el catálogo servicios relacionados (ej: HIFU, Microneedling, PRP también son bioestimuladores).
   - Si el usuario menciona un PRECIO (ej: "vi una promo de $100"), BUSCA en las promociones activas qué servicio coincide con ese precio y asume que se refiere a ese.
   - CORRIGE AMABLEMENTE: "Ah, entiendo. Usted se refiere a la promoción de [Nombre Servicio] que tiene ese valor..."
...
❓ **Si preguntan si la CONSULTA está INCLUIDA o es GRATIS:**
   → Aclara: "La consulta de evaluación tiene un valor de $10."
   → 🚨 **CIERRE OBLIGATORIO**: "Sin embargo, este valor se ABONA al 100% al costo de su tratamiento si decide realizárselo. Es decir, la consulta le sale GRATIS si se trata."
...
- 🔍 **BUSCA SINÓNIMOS**: Si usuario menciona "bioestimuladores", busca todos los tratamientos que estimulan colágeno (HIFU, Microneedling, PRP, Radiesse, Sculptra)
