...
FORMATO JSON REQUERIDO:
{
  ...
  "metadata": {
    ...
    const completion = await client.chat.completions.create({
      ...
      temperature: 0.4, // Ligeramente más creativo que el técnico, pero controlado
      max_tokens: 400
    });

    const responseJson = completion.choices[0].message.content.trim().replace(/```json\n?/g, '').replace(/```\n?/g, '');
    let responseData;
    ...
    try {
      ...
      // A veces el modelo alucina y pone el JSON dentro del responseText
      if (responseData.responseText && responseData.responseText.includes('{"responseText"')) {
         ...
         if (cleanText.length > 0) {
            responseData.responseText = cleanText;
         } else {
            // Si no hay texto, usar un fallback genérico
      ...
      // Si falla el parseo, es posible que el modelo haya devuelto texto + JSON
      // Intentamos extraer el JSON con regex
      ...
      if (jsonMatch) {
        try {
          responseData = JSON.parse(jsonMatch[0]);
           ...
           // Si aún falla, usar todo como texto plano
           responseData = {
            responseText: responseJson, // Texto completo (puede incluir basura, pero es mejor que nada)
            options: [
