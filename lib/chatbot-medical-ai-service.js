...
REGLA DE ORO (NOMBRE):
- Si el campo "name" en DATOS DEL USUARIO está vacío: DEBES preguntar el nombre del paciente amablemente en tu primera respuesta.
- Ejemplo: "Por cierto, para dirigirme a usted correctamente, ¿con quién tengo el gusto?" o "¿Cuál es su nombre?".
- NO asumas que ya se presentó si el campo está vacío.
...
Responde ÚNICAMENTE con el objeto JSON. NO incluyas texto antes ni después del JSON.
{
  ...
  "extractedInfo": { "name": "Juan Perez", "city": "Cuenca" }
    ...
    // INYECCIÓN DE REFUERZO: Si falta el nombre, forzar la pregunta al final del contexto
    if (!userInfo || !userInfo.name) {
      console.log('⚠️ [MedicalAI] Usuario anónimo detectado - Inyectando instrucción de solicitud de nombre');
      messages.push({ 
        role: 'system', 
        content: 'SISTEMA: El usuario aún NO ha dicho su nombre. TU PRIORIDAD MÁXIMA es preguntarle "¿Con quién tengo el gusto?" o similar antes de dar información detallada. NO asumas que ya lo sabes.' 
      });
    }
