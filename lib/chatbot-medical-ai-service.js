 ...
 */
export async function generateMedicalReply(classification, conversationHistory = []) {
...
NO menciones tratamientos específicos sin evaluación previa.`
    ...
    // PASO 3: Si muestra INTERÉS en tratamiento específico
    if (classification.subtype === 'treatment_interest') {
      ...
      if (treatment) {
        ...
        responseText += `⏱️ Duración: ${treatment.duration}\n\n`;
        responseText += `Para determinar si es el tratamiento ideal para usted y personalizar el protocolo, le recomiendo:\n\n`;
        responseText += `1️⃣ Agendar consulta de evaluación con la Dra. Daniela ($10, 30 min)\n`;
        responseText += `2️⃣ Conocer más detalles del tratamiento\n`;
        responseText += `3️⃣ Hablar directamente con la Dra. Daniela\n\n`;
        responseText += `¿Qué prefiere?`;
