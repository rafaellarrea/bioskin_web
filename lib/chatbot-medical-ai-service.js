...
NO menciones tratamientos específicos sin evaluación previa.`
    ...
    if (classification.subtype === 'price_inquiry') {
      ...
      // 🔍 NUEVO: Detectar si es consulta GENERAL sobre categoría (bioestimuladores, láser, etc.)
      const isGeneralCategoryQuery = /(bioestimulador|bioestimuladores|estimulador|l[aá]ser|tratamiento.*facial|tratamiento.*corporal|inyectable)/i.test(lastUserMsg) &&
                                     !/\b(hifu|microneedling|hollywood|ipl|co2|sculptra|radiesse)\b/i.test(lastUserMsg);
      
      if (isGeneralCategoryQuery) {
        console.log('📋 [MedicalAI] Consulta GENERAL sobre categoría, delegando a IA con contexto completo');
        // NO usar lógica predefinida, dejar que IA procese con contexto completo
        // Continuar al PASO 5 más abajo
      } else {
        // Buscar tratamiento ESPECÍFICO en catálogo
        const treatment = findServiceByKeyword(lastUserMsg);
        
        if (treatment) {
          console.log(`✅ [MedicalAI] Tratamiento específico encontrado: ${treatment.title}`);
      ...
      // Si no encontró tratamiento específico O es consulta general, continuar al PASO 5 con IA
    }
