...
function buildRealTimeUnsplashUrl(searchQuery) {
  ...
  // ✅ CAMBIO: Usar imágenes específicas de Unsplash con mejor confiabilidad
  const timestamp = Date.now();
  const randomSeed = Math.floor(Math.random() * 1000);
  
  // Usar múltiples estrategias para obtener imágenes variadas
  const strategies = [
    `https://images.unsplash.com/photo-1556909114-14e8ec2fec52?ixlib=rb-4.0.3&auto=format&fit=crop&w=1200&h=600&q=80&sig=${randomSeed}`, // Medical/aesthetic base
    `https://images.unsplash.com/photo-1559757148-5c350e09d4c6?ixlib=rb-4.0.3&auto=format&fit=crop&w=1200&h=600&q=80&sig=${randomSeed}`, // Skincare treatment
    `https://images.unsplash.com/photo-1512290923902-8a9f81dc236c?ixlib=rb-4.0.3&auto=format&fit=crop&w=1200&h=600&q=80&sig=${randomSeed}`, // Medical equipment
    `https://images.unsplash.com/photo-1582750433449-648ed127bb54?ixlib=rb-4.0.3&auto=format&fit=crop&w=1200&h=600&q=80&sig=${randomSeed}`, // Aesthetic clinic
    `https://images.unsplash.com/photo-1576091160399-112ba8d25d1f?ixlib=rb-4.0.3&auto=format&fit=crop&w=1200&h=600&q=80&sig=${randomSeed}`  // Medical technology
  ];
  
  // Seleccionar estrategia basada en keywords
  let selectedStrategy = 0;
  if (medicalContext.includes('laser') || medicalContext.includes('equipment')) {
    selectedStrategy = 2;
  } else if (medicalContext.includes('skincare') || medicalContext.includes('treatment')) {
    selectedStrategy = 1;
  } else if (medicalContext.includes('technology') || medicalContext.includes('device')) {
    selectedStrategy = 4;
  } else if (medicalContext.includes('clinic') || medicalContext.includes('aesthetic')) {
    selectedStrategy = 3;
  }
  
  const imageUrl = strategies[selectedStrategy];
  
  console.log(`🔍 Búsqueda en tiempo real: "${medicalContext}" → Strategy ${selectedStrategy} → ${imageUrl}`);
...
// ✅ NUEVA FUNCIÓN: Validar que las URLs de imágenes funcionan
...
// ✅ NUEVA FUNCIÓN: Obtener imagen con fallback garantizado
export async function getReliableImageUrl(searchQuery, fallbackStrategy = 0) {
  const primaryUrl = buildRealTimeUnsplashUrl(searchQuery);
  ...
  const fallbackImages = [
    'https://images.unsplash.com/photo-1556909114-14e8ec2fec52?ixlib=rb-4.0.3&auto=format&fit=crop&w=1200&h=600&q=80',
    'https://images.unsplash.com/photo-1559757148-5c350e09d4c6?ixlib=rb-4.0.3&auto=format&fit=crop&w=1200&h=600&q=80',
    'https://images.unsplash.com/photo-1582750433449-648ed127bb54?ixlib=rb-4.0.3&auto=format&fit=crop&w=1200&h=600&q=80'
  ];
  
  console.log(`⚠️ Usando imagen de fallback para: "${searchQuery}"`);
  return fallbackImages[fallbackStrategy] || fallbackImages[0];
}