...
function generateKeywordsFromContent(title, category, content = '') {
  ...
  // Encontrar la mejor coincidencia de keywords
  ...
  const generalKeywords = [
    'acido hialuronico', 'botox', 'rellenos', 'laser', 'ipl', 'hifu', 
    'radiofrecuencia', 'exosomas', 'plasma', 'hidratacion', 'manchas',
    'antienvejecimiento', 'limpieza', 'diagnostico', 'equipos'
  ];
  
  for (const keyword of generalKeywords) {
    if (titleLower.includes(keyword) || contentLower.includes(keyword)) {
      return categoryKeywords[keyword] || categoryKeywords.default;
    }
  }
...
// Función para construir URL de Unsplash
function buildUnsplashUrl(keywords, orientation = 'landscape') {
  const query = keywords.join(',');
  const baseUrl = 'https://images.unsplash.com/photo-';
  
  // IDs de fotos relacionadas con medicina estética y tecnología médica de alta calidad
  const medicalAestheticsPhotos = [
    '1559757148-5c350e0a9c6e', // Skincare
    '1556909114-f6e7ad7d3136', // Medical equipment
    '1576091160399-112ba8d25d1f', // Beauty treatment
    '1559757175-070c741b80eb', // Aesthetic clinic
    '1585652906653-f7d564bc68c2', // Medical technology
    '1573987791518-24d40e9e7d17', // Dermatology
    '1582750433449-648ed127bb54', // Beauty clinic
    '1512496015851-a90fb38ba796', // Medical procedure
    '1571019613454-1cb2f99b2d8b', // Aesthetic treatment
    '1588776814546-1ffcf47267a5'  // Medical aesthetics
  ];
  
  // Seleccionar una foto aleatoria
  const randomPhoto = medicalAestheticsPhotos[Math.floor(Math.random() * medicalAestheticsPhotos.length)];
  
  return `${baseUrl}${randomPhoto}?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D&auto=format&fit=crop&w=1200&h=600&q=80`;
}
...
export function generateBlogImage(blog) {
  ...
  try {
    ...
    // Construir URL de Unsplash
    const imageUrl = buildUnsplashUrl(keywords);
    
    console.log(`Generated image for blog "${title}":`, imageUrl);
