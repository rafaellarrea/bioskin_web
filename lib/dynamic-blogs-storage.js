...
// Función para inicializar blogs predeterminados
function initializeDefaultBlogs() {
  if (typeof window !== 'undefined') return; // Solo en servidor
  
  try {
    const defaultBlogsPath = join(__dirname, '../data/blogs-generated.json');
    if (existsSync(defaultBlogsPath)) {
      const defaultBlogsData = readFileSync(defaultBlogsPath, 'utf8');
      const defaultBlogs = JSON.parse(defaultBlogsData);
      
      // Agregar blogs predeterminados si el array está vacío
      if (dynamicBlogs.length === 0) {
        dynamicBlogs = [...defaultBlogs];
        console.log(`Initialized with ${defaultBlogs.length} default blogs`);
      } else {
        // Agregar solo blogs que no existan
        defaultBlogs.forEach(defaultBlog => {
          if (!dynamicBlogs.find(blog => blog.id === defaultBlog.id)) {
            dynamicBlogs.push(defaultBlog);
          }
        });
      }
    }
...
// Inicializar blogs predeterminados al cargar el módulo
...
export function getDynamicBlogs() {
  // Asegurar que siempre tengamos blogs predeterminados
  if (dynamicBlogs.length === 0 && typeof window === 'undefined') {
    initializeDefaultBlogs();
  }
  return [...dynamicBlogs];
}
...
export function addDynamicBlog(blog) {
  // Debug: Verificar que la imagen se está pasando
  console.log('🖼️ Agregando blog con imagen:', blog.image);
